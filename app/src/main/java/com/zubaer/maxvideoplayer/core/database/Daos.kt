...
interface MediaHistoryDao {
    ...
    @Query("SELECT * FROM media_history ORDER BY lastPlayedAtMs DESC")
    fun all(): Flow<List<MediaHistoryEntity>>
...
interface FavouriteDao {
    @Query("SELECT * FROM favourites ORDER BY addedAtMs DESC")
    fun observeAll(): Flow<List<FavouriteEntity>>

    @Query("SELECT stableMediaId FROM favourites")
    suspend fun ids(): List<String>

    @Upsert
    suspend fun upsert(entity: FavouriteEntity)

    @Query("DELETE FROM favourites WHERE stableMediaId = :stableMediaId")
    suspend fun delete(stableMediaId: String)
}

@Dao
interface PlaylistDao {
    @Query("SELECT * FROM playlists ORDER BY updatedAtMs DESC, name COLLATE NOCASE")
    ...
    @Query("DELETE FROM playlist_items WHERE playlistId = :playlistId AND stableMediaId = :stableMediaId")
    suspend fun deleteItem(playlistId: Long, stableMediaId: String)

    @Query("UPDATE playlist_items SET orderIndex = :orderIndex WHERE playlistId = :playlistId AND stableMediaId = :stableMediaId")
    suspend fun updateOrder(playlistId: Long, stableMediaId: String, orderIndex: Int)
}

@Dao
interface LibrarySourceDao {
    @Query("SELECT * FROM library_sources ORDER BY addedAtMs ASC")
    fun observeAll(): Flow<List<LibrarySourceEntity>>

    @Query("SELECT * FROM library_sources ORDER BY addedAtMs ASC")
    suspend fun all(): List<LibrarySourceEntity>

    @Query("SELECT * FROM library_sources WHERE id = :id LIMIT 1")
    suspend fun get(id: String): LibrarySourceEntity?
...
interface ExcludedFolderDao {
    @Query("SELECT * FROM excluded_folders ORDER BY displayName COLLATE NOCASE")
    fun observeAll(): Flow<List<ExcludedFolderEntity>>

    @Query("SELECT folderKey FROM excluded_folders")
    suspend fun keys(): List<String>

    @Upsert
    suspend fun upsert(entity: ExcludedFolderEntity)

    @Query("DELETE FROM excluded_folders WHERE folderKey = :folderKey")
    suspend fun delete(folderKey: String)
}

@Dao
interface MediaIndexDao {
    @Query("SELECT * FROM media_index ORDER BY title COLLATE NOCASE")
    fun observeAll(): Flow<List<MediaIndexEntity>>

    @Query("SELECT * FROM media_index WHERE stableMediaId = :stableMediaId LIMIT 1")
    suspend fun get(stableMediaId: String): MediaIndexEntity?

    @Query("SELECT * FROM media_index WHERE sourceId = :sourceId")
    suspend fun bySource(sourceId: String): List<MediaIndexEntity>

    @Upsert
    suspend fun upsertAll(items: List<MediaIndexEntity>)

    @Query("UPDATE media_index SET availability = 'UNAVAILABLE' WHERE sourceId = :sourceId")
    suspend fun markSourceUnavailable(sourceId: String)

    @Query("DELETE FROM media_index WHERE sourceId = :sourceId")
...
interface LibraryPreferenceDao {
    @Upsert
    suspend fun upsert(entity: LibraryPreferenceEntity)

    @Query("SELECT * FROM library_preferences WHERE `key` = :key LIMIT 1")
    suspend fun get(key: String): LibraryPreferenceEntity?
}
