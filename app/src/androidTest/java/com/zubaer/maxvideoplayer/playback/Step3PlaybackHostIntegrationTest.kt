...
class Step3PlaybackHostIntegrationTest {
    ...
    fun serviceQueueSurvivesNavigationFullscreenRecreationAndPip() {
        ...
        try {
            ...
            assertTrue("Next did not publish the coherent C queue item", await(5_000L) {
                val state = connection.state.value
                (state.mediaId == queue[2].stableId && state.title == "Step 3 Queue C") || state.error != null
            })
            assertNull("Next navigation failed: ${connection.state.value.error}", connection.state.value.error)
            assertEquals(queue[2].stableId, connection.state.value.mediaId)
            assertEquals("Step 3 Queue C", connection.state.value.title)

            instrumentation.runOnMainSync { connection.seekToPrevious() }
            assertTrue("Previous did not publish the coherent B queue item", await(5_000L) {
                val state = connection.state.value
                (state.mediaId == queue[1].stableId && state.title == "Step 3 Queue B") || state.error != null
            })
            assertNull("Previous navigation failed: ${connection.state.value.error}", connection.state.value.error)
            assertEquals(queue[1].stableId, connection.state.value.mediaId)
